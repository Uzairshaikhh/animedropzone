...
async function sendViaMailerSend(to: string, subject: string, html: string) {
  ...
  let mailersendApiKey = Deno.env.get('MAILERSEND_API_KEY') 
    || Deno.env.get('mailsender_api') 
    || Deno.env.get('mail_api');
  ...
  let fromEmail = Deno.env.get('MAILERSEND_FROM_EMAIL') || Deno.env.get('mail_sender') || 'info@test-zkq340endq0gd796.mlsender.net';
...
async function sendViaHostinger(to: string, subject: string, html: string) {
  const hostingerEmail = Deno.env.get('HOSTINGER_EMAIL'); // Your full email address (e.g., noreply@yourdomain.com)
  const hostingerPassword = Deno.env.get('HOSTINGER_PASSWORD'); // Your email password
