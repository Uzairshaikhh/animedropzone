...
import { X, Plus, Trash2, Edit2, Lock, User, ArrowLeft, Upload, Image as ImageIcon, Package, ShoppingBag, Tag, Percent, Shirt, DollarSign, CheckCircle, CreditCard, MessageSquare, FolderOpen, Download, Mail, Users, BarChart3, Bell, FileSpreadsheet, Award, Search, RotateCcw, Shield } from 'lucide-react';
...
export function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'orders' | 'payments' | 'coupons' | 'custom-clothing' | 'support' | 'categories' | 'wallpapers' | 'email' | 'users' | 'analytics' | 'inventory-alerts' | 'bulk-operations' | 'newsletter-management' | 'loyalty-program' | 'returns' | 'legal-content'>('products');
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {isLoggedIn && (
                <>
                  ...
                  <button
                    onClick={async () => {
                      if (!confirm('Seed default categories and wallpapers? This will only add them if none exist.')) {
                        return;
                      }
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm">
                                ...
                                <span className="text-gray-400 capitalize hidden sm:inline">Category: {product.category}</span>
                                {(product as any).subcategory && (
                                  <span className="text-gray-400 capitalize hidden lg:inline">Subcategory: {(product as any).subcategory}</span>
                                )}
                              </div>
                            </div>
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 text-sm mb-4">
                              ...
                              <span className="text-gray-400">Created At: {new Date(order.createdAt).toLocaleString()}</span>
                            </div>
                                ...
                                onChange={async (e) => {
                                  ...
                                  if (confirm(`Update order status to "${newStatus}"?`)) {
                                    try {
                                      ...
                                      const data = await response.json();
                                      if (data.success) {
                                        alert('✅ Order status updated successfully! Customer will be notified via Email and WhatsApp.');
                            ...
                            <div className="mt-4">
                              ...
                              {(order as any).paymentType && (
                                <p className="text-gray-400 text-sm mb-1 flex items-center gap-2">
                                  Payment Type: 
                                  <span className={`px-2 py-0.5 rounded-full text-xs ${(order as any).paymentType === 'Prepaid' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30'}`}>
                                    {(order as any).paymentType}
                                  </span>
                                </p>
                              )}
                              {(order as any).paymentStatus && (
                                <p className="text-gray-400 text-sm mb-1 flex items-center gap-2">
                                  Payment Status: 
                                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${(order as any).paymentStatus === 'Paid' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-orange-500/20 text-orange-400 border border-orange-500/30'}`}>
                              ...
                              {(order.paymentMethod.toLowerCase().includes('razorpay') || order.paymentMethod.toLowerCase().includes('paytm')) && order.status !== 'Cancelled' && (
                                <div className="mt-4 p-4 bg-gradient-to-br from-yellow-900/20 to-orange-900/20 border border-yellow-500/30 rounded-lg">
                                  ...
                                  <p className="text-gray-300 text-sm mb-3">
                                    This order was placed using {order.paymentMethod}. Please verify that payment was actually received before processing the order.
                                  </p>
                                    ...
                                    <div className="space-y-2">
                                      <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-3 mb-3">
                                        ...
                                        <p className="text-gray-400 text-xs">
                                          Check your {order.paymentMethod} dashboard to confirm payment was received for Payment ID: <span className="text-purple-400 font-mono">{order.paymentId}</span>
                                        </p>
                                      </div>
                                      ...
                                      <div className="flex gap-2">
                                        <button
                                          onClick={async () => {
                                            if (confirm(`Confirm that you have verified payment receipt in your ${order.paymentMethod} dashboard?\n\nPayment ID: ${order.paymentId}\nAmount: ₹${order.total.toLocaleString()}\n\nThis will mark the payment as verified and allow order processing.`)) {
                                              try {
                                                const response = await fetch(
                                            ...
                                            if (reason && reason.trim()) {
                                              if (confirm(`Are you absolutely sure you want to CANCEL this order?\n\nReason: ${reason}\n\nThe customer will be notified via email.`)) {
                                                try {
                                                  ...
                                                  const data = await response.json();
                                                  if (data.success) {
                                                    alert('✅ Order cancelled successfully. Customer has been notified via email.');
                                                    fetchOrders(); // Refresh orders
                                                  } else {
