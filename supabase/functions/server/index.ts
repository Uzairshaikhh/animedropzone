...
app.get("/make-server-95a96d8e/kv-status", async (c) => {
  try {
    ...
    console.log(
      "✅ wallpapers_array:",
      combined ? `${Array.isArray(combined) ? combined.length : "not-array"} items` : "null"
    );
...
// Get all wallpapers (using Deno KV)
app.get("/make-server-95a96d8e/wallpapers", async (c) => {
  try {
    console.log("🔵 GET /wallpapers - Fetching from Deno KV...");

    const kv = await Deno.openKv();
    const wallpapersList: any[] = [];

    // Get all wallpapers from KV
    const iter = kv.list({ prefix: ["wallpaper"] });
    for await (const res of iter) {
      if (res.value) {
        wallpapersList.push(res.value);
      }
    }

    // Sort by order
    wallpapersList.sort((a, b) => (a.order || 0) - (b.order || 0));

    console.log("✅ Fetched", wallpapersList.length, "wallpapers from KV");
    return c.json({ success: true, wallpapers: wallpapersList });
  } catch (error) {
...
app.post("/make-server-95a96d8e/wallpapers", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("wallpapers").insert([wallpaper]).select();
    ...
    const defaultCategories = [
      {
        ...
        icon: "Package",
        subcategories: ["Tanjiro", "Nezuko", "Hashiras", "Demons"],
      },
      {
        name: "Naruto Figures",
        slug: "naruto",
        ...
        icon: "Package",
        subcategories: ["Main Characters", "Hokages", "Team 7", "Villains"],
      },
      {
        ...
        icon: "Package",
        subcategories: ["Luffy & Crew", "Straw Hat Pirates", "Antagonists", "Side Characters"],
      },
      {
        ...
        icon: "Package",
        subcategories: ["Eren & Friends", "Survey Corps", "Titans", "Antagonists"],
      },
      {
        ...
        icon: "Package",
        subcategories: ["Deku & Class 1-A", "All Might & Heroes", "Villains", "Support Characters"],
      },
      {
        ...
        icon: "Package",
        subcategories: ["Goku & Vegeta", "Super Saiyans", "Fusion Forms", "Villains"],
      },
      {
        name: "Katana",
        slug: "katana",
